...
SECRET_KEY = "your-secret-key" # set and store strong key
ALGORITHM = "HS256"
ACCESS_TOKEN_EXPIRE_MINUTES = 60 # TODO: we have to change this before deploy
...
# TODO: [debug this function later in the day (mabr3) ]    
# Update password 
...
def chat(
    ...
    print("Received Message : "+ message) # For debugging purpose
    ...
    # Send message to llama to process
...
def get_conversion(message:str = Body(...)):
    ...
    try:
        response = requests.post("http://localhost:11434/api/generate", json={
        "model": "llama3.2", # Switch to physics-llama before deploy
        "prompt": message,
        "stream": False
        }).json()["response"]
